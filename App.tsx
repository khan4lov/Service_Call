...
const App: React.FC = () => {
  ...
  const handleAcceptBooking = async (bookingId: string) => {
      ...
      setBookings(prev => prev.map(b => 
          b.id === bookingId 
          ? { ...b, status: 'ASSIGNED', providerId: currentUser.username } 
          : b
      ));
  ...
  const handleAssignBooking = async (bookingId: string, providerUsername: string) => {
      setBookings(prev => prev.map(b => 
        b.id === bookingId 
        ? { ...b, status: 'ASSIGNED', providerId: providerUsername } 
        : b
      ));
      await api.updateBooking(bookingId, { status: 'ASSIGNED', providerId: providerUsername });
  };
